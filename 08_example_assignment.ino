...
float dist_min, dist_max, dist_raw, dist_raw_backup; // unit: mm
...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  if(dist_raw == 150 || dist_raw == 250) {
    ...
    analogWrite(PIN_LED, 254 - 2.43 * (dist_raw - 100));
  }
  else if(dist_raw > 200 && dist_raw < 300){
    analogWrite(PIN_LED, 2.43 * (dist_raw - 200));
  }
  else {
